...
const SuggestedArticles = ({ classname, header, data, tags }: Props) => {
  return (
    <div className={`w-full shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px] rounded-lg p-4 ${classname}`}>
        ...
        {data.map((item) => (
          <div key={item._id} className="flex space-x-3 flex-nowrap items-center">
            ...
            <div className="text-sm text-primary2 font-medium">
              <h3 className='text-sm text-primary2 fomt-medium md:text-base lg:text-lg'>{item.title}</h3>
              <span className="text-xs opacity-60">{new Date(item.createdAt).toDateString()}</span>
            </div>
          </div>
        ))}
      </div>
